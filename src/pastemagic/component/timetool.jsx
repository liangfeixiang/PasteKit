...
export default function TimeTool({content}) {
    ...
    const convertContent = () => {
        ...
        // 其他类型
        return {
            display: content != null ? String(content) : '(空内容)',
            timestamp: null,
            type: 'other',
            showDate: false,
            showTimestamp: false
    ...
    const formatTimestampDisplay = (timestamp) => {
        ...
        if (clean.toString().length === 10) {
            ...
            return [
                { label: '时间戳(s)', value: timestamp },
                { label: '时间戳(ms)', value: millisecondTimestamp }
            ];
        } else if (clean.toString().length === 13) {
            // 13位时间戳：显示毫秒级，同时计算对应的秒级
            const secondTimestamp = Math.floor(timestamp / 1000);
            return [
                { label: '时间戳(ms)', value: timestamp },
                { label: '时间戳(s)', value: secondTimestamp }
            ];
        }
        return [{ label: '时间戳', value: timestamp }];
    };
